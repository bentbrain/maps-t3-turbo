...
export const RIGHT_SIDEBAR_WIDTH = "30rem";
...
interface MultiSidebarContextType {
...
function MultiSidebarProvider({
  ...
  // Add useEffect to update cookie when leftOpen changes
  React.useEffect(() => {
    document.cookie = `${SIDEBAR_COOKIE_NAME}:left=${leftOpen}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`;
  }, [leftOpen]);
  ...
  // Add useEffect to update cookie when rightOpen changes
  React.useEffect(() => {
    document.cookie = `${SIDEBAR_COOKIE_NAME}:right=${rightOpen}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`;
  }, [rightOpen]);
    ...
    return (
      <div
        ...
        className={cn(
          ...
          side === "left" ? "w-(--sidebar-width)" : "w-(--right-sidebar-width)",
          className,
        )}
        {...props}
      >
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-mobile="true"
          className={cn(
            "text-sidebar-foreground w-(--sidebar-width) border-none bg-transparent bg-none p-2 [&>button]:hidden",
            {
              "pr-0": side === "left",
              "pl-0": side === "right",
            },
          ...
          side={side}
        >
          <div className="bg-sidebar/90 flex h-full flex-col gap-4 rounded-md backdrop-blur-sm">
...
>(
  (
    ...
    const { isMobile, state, toggleSidebar } = sidebarContext;
